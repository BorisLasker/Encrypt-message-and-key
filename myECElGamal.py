...
def Initiation_gamal():
    ...
    return a,b,base_point,mod,order
    #------------------------------------
    #symmetric encryption

def Encryption_key_gamal(message,a,b,base_point,mod,order):
    arrc1=[]
    arrc2=[]
    for i in range(0, len(message)):
        ...
        plaintext = textToInt(message[i])
    
        #print("message: ",message,". it is numeric matching is ",plaintext)
        
        #transforms the int plaintext to coordinates on the eliptic curve
...
def Decryption_key_gamal(c1,c2,a,b,mod,base_point,order):
    res=[]
    for j in range(0, len(c1)):
        decryption_begins = time.time()
        
        #secret key times c1
        dx, dy = EccCore.applyDoubleAndAddMethod(c1[j][0], c1[j][1], secretKey, a, b, mod)
        #-secret key times c1
        dy = dy * -1 #curve is symmetric about x-axis. in this way, inverse point found
        
        #c2 + secret key * (-c1)
        decrypted = EccCore.pointAddition(c2[j][0], c2[j][1], dx, dy, a, b, mod)
